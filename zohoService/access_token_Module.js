...
const get_access_token = async() =>{
    ...
    if(refresh_token){
        const token = await axios.post(`https://accounts.zoho.in/oauth/v2/token?refresh_token=${refresh_token}&client_id=${envVariables.CLIENT_ID}&client_secret=${envVariables.CLIENT_SECRET}&redirect_uri=https://www.zoho.com/books/&grant_type=refresh_token`)
        .then(resp=>{
    ...
    //    logger.warn("refresh token is not found in File System"," correlationId Id: ",correlationId, " patient uhid: ",uhid);
        const token = await axios.post(`https://accounts.zoho.in/oauth/v2/token?code=${envVariables.CODE}&client_id=${envVariables.CLIENT_ID}&client_secret=${envVariables.CLIENT_SECRET}&redirect_uri=https://www.zoho.com/books/&grant_type=authorization_code`)
        .then(res =>{
