...
zohoServices.invoice = async(uuid, items_list, userObj, order_id) =>{
    try {
        ...
        if(token){
            ...
            if(token_timer && ((Date.now()) - token_timer < (token.expires_in-300)*1000)){
                logger.info("token without any API call ",token.access_token);
            }else{
                token = await get_access_token();
                logger.info("token after API call because expired ",token.access_token);
            }
        }else{
            token = await get_access_token();
            logger.info("token after API call ",token.access_token);
        }
        ...
        // Step 3 - Get user id from zoho Books or create a new user on the fly
        logger.trace("Step 3 started - Get user id from zoho Books or create a new user on the fly")
        let user_id = await axios.get(`https://www.zohoapis.in/books/v3/contacts?cf_uuid=${uuid}&process.env.organization_id=${process.env.organization_id}`,config)
        .then(res=>{
            if(res.data.contacts.length > 0){
                logger.info("Step 3 successfull - Get customer id from zoho Books")
                return res.data.contacts[0].contact_id;
            }
            else{
                logger.warn("couldn't find contact in books, creating contact...");
                return axios.post(`https://www.zohoapis.in/books/v3/contacts?process.env.organization_id=${process.env.organization_id}`,userObj,config).then(result=>{
                    logger.info("Step 3 successfull - create a new customer on the fly")
                    return result.data.contact.contact_id;
                }).catch(err=>{
                    logger.error("step 3 create user error " + err.message)
                    throw new Error("create user error, " + err.message);
                })
            }
        })
        .catch(error=>{
            logger.error("step 3 get user error " + error.message);
            throw new Error("get user error, " + error.message);
        })

        // Step 4 - get ids of all the requested items using concept_id provided
        logger.trace("Step 4 started - get ids of all the requested items using concept_id provided")
        let line_items = items_list.map(item =>{
            ...
            .then(result_res=>{
                ...
                logger.info("item found with concept_id ",item.conceptId);
                return {item_id,quantity:item.quantity}
                }
                logger.warn("no item found with concept_id "+ item.conceptId +" on order id =>"+order_id);
                throw new Error("no item found with concept_id "+item.conceptId);
            }).catch(err=>{
                logger.error("step 4 get items list error " + err.message +" on order id =>"+order_id)
        ...
        return Promise.all(line_items).then(async line_items_values=>{
            logger.info("line items ids ",line_items_values);
            ...
            logger.trace("Step 5 started - create invoice and get invoice id to use in send email api")
            ...
            .then(response_result =>{
                logger.info(response_result.data.message);
                return response_result.data;
            }).catch(err=>{
                logger.error("step 5 create invoice error " + err.message +" on order id =>"+order_id)
            ...
            logger.trace("Step 6 started- Email the generated invoice to the customer for further processing")
            ...
            .then(res=>{
                ...
                logger.error("Email sending to user error, " + err.message +" on order id =>"+order_id)
                throw new Error("Email sending to user error, " + err.message);
            })

            if(success_msg){
                let invoice_no = invoice_details.invoice.invoice_number
                return {invoice_no, order_id};
            }
        }).catch(error=>{
            logger.error("Promise resolve error for list of items array, " + error.message +" on order id =>"+order_id)
            throw new Error("Promise resolve error for list of items array, " + error.message +" on order id =>"+order_id);
        })
    } catch (error) {
        logger.error("before final Error");
